...
export function CommentsSection() {
  ...
  function submit(event: FormEvent) {
    ...
    setLoading(true);
    const comment: CommentEntry = { id: createId(), author, eventType: draft.eventType, body, createdAt: new Date().toISOString(), isSeed: false, ownerToken };
  ...
  function finishSwipe(event: TouchEvent<HTMLDivElement>) {
    ...
    if (!start || orderedComments.length < 2) return;
    ...
    if (horizontalDistance > 0) showPreviousComment();
    else showNextComment();
  }
